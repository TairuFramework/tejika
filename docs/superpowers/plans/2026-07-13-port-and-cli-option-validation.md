...
- The one behaviour worth eyeballing by hand: with `withPort` on a parent command, a leaf action must read `optsWithGlobals()`, not `opts()`. This was already true before the change (a leaf reading `opts().port` saw `undefined`); the fix makes the value actually arrive on the parent.
