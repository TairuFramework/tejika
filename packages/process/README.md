...
```sh
pnpm add @tejika/process @enkaku/server
```

`runDaemon`'s boot callback returns an Enkaku `Server`, so the host app also
depends on `@enkaku/server` directly (see the example below).
