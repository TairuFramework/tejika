...
- Two upstream `@enkaku/socket` gaps (no connect timeout + leaked listeners; a dispose that only `unref()`s and skips function sources entirely) are written up in the Enkaku repo's own `docs/agents/plans/next/`.
